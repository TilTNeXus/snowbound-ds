...
void playBGM(char song[]) {
    if (strcmp(song, "mainmenu") == 0) {
		...
		if ((screenFrames-musOffset)%(8*chunkLength) == 1) {
	  		...
	  		if (!NF_RAWSOUND[1].available) NF_UnloadRawSound(1);
	  		NF_LoadRawSound("audio/music/secilytheme.1", 1, 11025, 0);
	  		NF_PlayRawSound(1, 127, 64, false, 0);
	 		...
	 		NF_PlayRawSound(0, 127, 64, false, 0);
	  		...
	  		if (!NF_RAWSOUND[1].available) NF_UnloadRawSound(1);
	  		NF_LoadRawSound("audio/music/secilytheme.3", 1, 11025, 0);
	 		NF_PlayRawSound(1, 127, 64, false, 0);
			...
			if (!NF_RAWSOUND[1].available) NF_UnloadRawSound(1);
			NF_LoadRawSound("audio/music/secilytheme.5", 1, 11025, 0);
		   	NF_PlayRawSound(1, 127, 64, false, 0);
			...
			if (!NF_RAWSOUND[1].available) NF_UnloadRawSound(1);
			NF_LoadRawSound("audio/music/secilytheme.7", 1, 11025, 0);
		   	NF_PlayRawSound(1, 127, 64, false, 0);
	  	}
	}
}
